...
class plgSystemRSFPRegistration extends JPlugin
{
	...
	public function rsfp_beforeUserEmail($args) {
		...
		if ($this->_getRow($form->FormId) && !$this->user) {
			$args['userEmail']['to'] = '';
		}
	}
		...
		if ($row = $this->_getRow($formId)) {
			...
			if ($useractivation == 1) {
				switch ($row->user_activation_action) {
					...
					case 1:
						$app->redirect($row->user_activation_url);
						break;
				...
				switch ($row->admin_activation_action) {
					...
					case 1:
						$app->redirect($row->admin_activation_url);
						break;
	...
	public function onAfterDispatch() {
		...
		if ($formId = RSFormProHelper::getConfig('registration_form')) {
			$app->redirect(JRoute::_('index.php?option=com_rsform&formId='.$formId, false));
		} elseif ($url = RSFormProHelper::getConfig('redirect_url')) {
			$app->redirect($url);
		}
	}
