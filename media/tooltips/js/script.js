	...
	$(document).ready(function() {
		...
		if (tooltips_use_auto_positioning) {
			$.fn.popover.Constructor.prototype.show = function() {
				var el = this;
				tooltipsFixPosition(el);

			}
		}

		function tooltipsFixPosition(el) {
			if (!el.hasContent() || !el.enabled) {
				return;
			}

			var e = $.Event('show.bs.' + el.type);
		...
		function tooltipsSetPosition(el) {
			var $tip = el.tip();
			...
			el.applyPlacement(tp, placement);
		}
	});
})(jQuery);
